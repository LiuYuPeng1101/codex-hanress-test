...
新增的实现、支持的部署拓扑、失败语义和未验收项目见 [可靠性与复用边界](RELIABILITY.md)。

本轮已实现：单进程会话互斥、总并发限制、HTTP/SSE 断线与执行解耦、有界订阅、停止接单和排空、评测失败/跳过门禁、真实 SDK 通知契约修复以及 Java RestClient starter 修复。以下原始审计中的“尚未具备”应结合这次更新阅读。

当前仍然不能直接称为完全生产就绪：真实 OMS 幂等、身份与密钥边界、真实模型评测和恢复演练仍待验收。并发和排空只覆盖文档指定的单实例部署，不是多副本能力。
